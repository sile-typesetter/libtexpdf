...
{
  verbose++;
  texpdf_subfont_set_verbose();
  texpdf_tfm_set_verbose();
  vf_set_verbose ();
  spc_set_verbose();
}
...
dvi_locate_font (const char *tfm_name, spt_t ptsize)
{
  ...
  name = kpse_find_tfm(tfm_name);
  if (!name)
    name = tfm_name; /* for error reporting */

  loaded_fonts[cur_id].tfm_id     = texpdf_tfm_open(name, 1);
...
dvi_set (int32_t ch)
{
  ...
  case  PHYSICAL:
    if (ch > 65535) { /* _FIXME_ */
      ...
      unsigned short uch = texpdf_lookup_sfd_record(font->subfont_id, (unsigned char) ch);
      wbuf[0] = (uch >> 8) & 0xff;
      wbuf[1] =  uch & 0xff;
      texpdf_dev_set_string(pdf, dvi_state.h, -dvi_state.v, wbuf, 2,
			 width, font->font_id, 2);
  ...
  case  VIRTUAL:
    ...
    if (font->subfont_id >= 0)
      ch = texpdf_lookup_sfd_record(font->subfont_id, (unsigned char) ch);
#endif /* 0 */
...
dvi_put (int32_t ch)
{
  ...
  case  PHYSICAL:
    ...
    if (ch > 65535) { /* _FIXME_ */
      ...
      uch = texpdf_lookup_sfd_record(font->subfont_id, (unsigned char) ch);
      wbuf[0] = (uch >> 8) & 0xff;
      wbuf[1] =  uch & 0xff;
      texpdf_dev_set_string(pdf, dvi_state.h, -dvi_state.v, wbuf, 2,
			 width, font->font_id, 2);
    ...
    if (!strcmp(q, "landscape")) {
      ...
      *owner_pw = *user_pw = 0;
	  ...
	  pdf_obj *obj;
	      ...
	      *key_bits = (unsigned) texpdf_number_value(obj);
	    } else
	      error = -1;
	    if (obj)
	      texpdf_release_obj(obj);
            ...
            if ((obj = texpdf_parse_pdf_number(&p, endptr)) && PDF_OBJ_NUMBERTYPE(obj)) {
	      *permission = (unsigned) texpdf_number_value(obj);
	    } else
	      error = -1;
	    if (obj)
	      texpdf_release_obj(obj);
